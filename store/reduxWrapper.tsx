...
export const createWrapper = <S extends Store>(makeStore: MakeStore<S>, config: Config<S> = {}) => {
  ...
  const makeProps = async ({
    ...
    const inStore = initStore({context, makeStore})

    if (config.debug) {
      console.log('1. getProps created store with state', inStore.getState())
    }

    // Legacy stuff - put inStore in context
    if (addStoreToContext) {
      if (context.ctx) {
        context.ctx.store = inStore
      } else {
        context.store = inStore
      }
    }

    const nextCallback = callback && callback(inStore)
    const initialProps = (nextCallback && (await nextCallback(context))) || {}

    if (config.debug) {
      console.log('3. getProps after dispatches has store state', inStore.getState())
    }

    const state = inStore.getState()
  ...
  const getInitialPageProps =
    <P extends object = any>(callback: PageCallback<S, P>): GetInitialPageProps<P> =>
      async (
  ...
  const getInitialAppProps =
    <P extends object = any>(callback: AppCallback<S, P>): GetInitialAppProps<P> =>
  ...
  const getStaticProps =
    <P extends object = any>(callback: GetStaticPropsCallback<S, P>): GetStaticProps<P> =>
      async context => {
        const {initialProps, initialState} = await makeProps({
          callback,
          context,
  ...
  const getServerSideProps =
    <P extends object = any>(callback: GetServerSidePropsCallback<S, P>): GetServerSideProps<P> =>
      async context =>
  ...
  const withRedux = (Component: NextComponentType|App|any) => {
    ...
    return class Wrapper extends React.Component<any, any> {
      ...
      hydrate({initialState, initialProps, ...props}: any, context: any) {
        ...
        if (!store) {
          store = initStore({makeStore, context})
        ...
        if (initialStateFromGSPorGSSR) {
          ...
          if (!getIsServer()) {
            waitingState.push(initialStateFromGSPorGSSR)
          }
        }
      }
  ...
  /**
   * This method will be called at store/index.ts:80
   * to make sure it dispatches HYDRATE correctly
   */
  const triggerHydrate = () => {
    const p = waitingState.pop()
    p && store.dispatch({
      type: HYDRATE,
      payload: getDeserializedState<S>(p, config),
    } as any)
  }
...
//FIXME Could be typeof App.getInitialProps & appGetInitialProps (not exported), see
// https://github.com/kirill-konshin/next-redux-wrapper/issues/412

  ...
  async function xls2Json(_path) {
    return new Promise((resolve, reject) => {
      try {
        ...
        XlsxPopulate.fromFileAsync(fileLocation).then(book => {
          let sheetName = ''
          try {
              ...
              if (refObj[i]) {
                ...
                for (let j in refs) {
                  ...
                  for (let lang in localeObj[i]) {
                    let commonFile = i18nPath + '/common/' + key + '.json',
                      obj = fs.readFileSync(commonFile, 'utf8')
                    obj = JSON.parse(obj)
                    Object.assign(localeObj[i][lang], { [key]: obj[lang] })
                  }
                }
              }

              // Write json file
              const _path2 = jsonDir + '/' + i + '.json'
              let writeFS = fs.createWriteStream(_path2)
              writeFS.write(JSON.stringify(localeObj[i]))
              console.log('build file JSON success by file', _path2)
            }
            resolve()
          }
          catch (error) {
